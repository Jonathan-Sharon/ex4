...
ThreadPool::Queue::Queue(const int sockfd, const sockaddr_in connectAddress,
    ...
    : m_mapCreator{std::make_unique<MapCreator::MapCreator>()},
      m_sockfd{sockfd}, m_connectAddress{connectAddress},
      m_numberOfThreads{numberOfThreads}, m_isSerial{numberOfThreads == 1},
      m_isThereSomethingToRead{false}, m_numberOfAvailableThreads{
                                           numberOfThreads - 1} {}
...
void ThreadPool::Queue::allocate() {
  while (true) {
    ...
    if (!m_writeQueue.empty()) {
      ...
      if (m_isSerial == true) {
        m_writeQueue.front().writer.get()->writeMessage(
            *this, m_writeQueue.front().parameters);
        m_writeQueue.pop();
        continue;
      }

      // The server is parallel. Since it's the only class that can create
      // thread, and according to the wait() function there is a thread
      // available then create a thread to do the Write operation. Also, update
      // the number of available threads.
      std::unique_lock<std::mutex> lock(m_writeQueueMutex);
      m_threadVector.push_back(
    ...
    if (!m_operateQueue.empty()) {
      ...
      // in the queue using the main thread.
      if (m_isSerial == true) {
        m_operateQueue.front().operation.get()->operate(
            *this, m_operateQueue.front().parameters);
        m_operateQueue.pop();
        continue;
      }
      ...
      // The server is parallel. Since it's the only class that can create
      // thread, and according to the wait() function there is a thread
      // available then create a thread to do the Operate operation. Also,
      // update the number of available threads.
    ...
    if (!m_readQueue.empty()) {
      ...
      // in the queue using the main thread.
      if (m_isSerial == true) {
        m_readQueue.front().reader.get()->readMessage(
            *this, m_readQueue.front().parameters);
        m_readQueue.pop();
        continue;
      }
      ...
      // The server is parallel. Since it's the only class that can create
      // thread, and according to the wait() function there is a thread
      // available then create a thread to do the Read operation. Also, update
      // the number of available threads.
  ...
  while (true) {
    ...
    if (m_numberOfAvailableThreads >= m_numberOfThreads) {
      throw std::runtime_error(
          "number of available operation is bigger then number of threads");
    }
...
void ThreadPool::Queue::checkActiveFd() {
  ...
  if (FD_ISSET(m_sockfd, &readfds)) {
    ...
    waitFroReadCreator.addToQueue(
        *this, {std::time(nullptr), VERSION, new_socket, "FIRST READ", ""});
  }
